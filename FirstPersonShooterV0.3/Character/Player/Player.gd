...
@onready var AmmoLabel =$Head/PlayerCam/Control/Label
...
func _process(delta: float) -> void:
	...
	if CurrentWeaponAmmo != -200:
		AmmoLabel.text = str(CurrentWeaponAmmo)
	else:
		AmmoLabel.text = ""
	print(ShotgunAmmo)
...
func _Attack():
	...
	if CurrentWeapon == Pistol and PistolAmmo > 0:
		WeaponCooldown.start(0.5)
		CurrentWeapon.play("Attack")
		PistolAmmo = PistolAmmo - 1
		Face.Face1 = 1
	elif CurrentWeapon == Shotgun and ShotgunAmmo > 0:

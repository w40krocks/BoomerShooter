...
func _ready():
	#begins the playing of the idle face animation
	face.play("default")
...
func _Shoot():
	#plays the shooting animations for the weapon and the player face
	Gun.play("Shoot")
	face.play("Angry")
	if Shot.is_colliding() == true:
		print("Hit a shot!")
		#prints what the player has hit
		print(Shot.get_collider())
		#if players shoot the health pickup add 25 health
...
func _HealthChange(health):
	...
	if Health > 100:
		Health = 100
	#change the size of the health representation circle based off the current health
